    ...
    <DashboardLayout>
      ...
      {displayed.length === 0 ? (
        <div className="rounded-2xl p-14 text-center" style={{ background:'var(--bg3)', border:'1px solid var(--border2)', boxShadow:'var(--shadow)' }}>
          ...
          <p style={{ color:'var(--text4)', fontSize:'0.8rem', marginTop:'4px' }}>Click "Raise Ticket" to submit a new issue</p>
        </div>
      ) : (
        <div className="space-y-3">
          {displayed.map((t, i) => {
            ...
            return (
              <motion.div key={t.id} style={card}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium"
                        style={priorityStyle[t.priority]}>{t.priority}</span>
                    </div>
                    <div className="flex items-center gap-3 flex-wrap" style={{ color:'var(--text4)', fontSize:'0.72rem' }}>
                      ...
                      {t.resolved_at && <span style={{ color:'#15803D' }}>Resolved {new Date(t.resolved_at).toLocaleDateString()}</span>}
                    </div>
                  </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Raise a Support Ticket">
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block mb-1.5" style={lbl}>Category *</label>
              <select className="input-dark" value={form.category} onChange={set('category')}>
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="block mb-1.5" style={lbl}>Priority *</label>
          ...
          <div>
            <label className="block mb-1.5" style={lbl}>Detailed Description *</label>
          ...
          <div className="flex items-start gap-2 p-3 rounded-xl"
            ...
            <p style={{ color:'var(--pd)', fontSize:'0.78rem' }}>Your ticket will be reviewed by the admin. You'll see their response here once they reply.</p>
          </div>
          <button type="submit" className="btn-primary w-full flex items-center justify-center gap-2">
            <HeadphonesIcon size={15}/> Submit Ticket
          </button>
        </form>
      </Modal>
    </DashboardLayout>
