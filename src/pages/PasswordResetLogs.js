...
export default function PasswordResetLogs() {
  ...
  useEffect(() => {
    api.get('/auth/reset-logs').then(d => setLogs(Array.isArray(d) ? d : [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
    ...
    <DashboardLayout>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl p-14 text-center" style={{ background:'var(--bg3)', border:'1px solid var(--border2)', boxShadow:'var(--shadow)' }}>
          ...
          <p style={{ color:'var(--text3)', fontSize:'0.8rem', marginTop:'4px' }}>When students reset their passwords, it will appear here</p>
        </div>
      ) : (
            ...
            <motion.div key={log.id || i}
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 flex-wrap mb-1">
                  <span className="flex items-center gap-1.5 font-semibold text-sm" style={{ color:'var(--text1)' }}>
                ...
                <div className="flex items-center gap-1.5" style={{ color:'var(--text4)', fontSize:'0.72rem' }}>
                  <Clock size={11}/>
                  {new Date(log.reset_at).toLocaleString('en-IN', { day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })}
                </div>
              </div>
            </motion.div>
          ))}
