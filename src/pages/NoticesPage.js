...
export default function NoticesPage() {
  ...
  const load = () => api.get('/notices').then(d => setNotices(Array.isArray(d) ? d : [])).catch(() => {});
  useEffect(() => { load(); }, []);
  ...
  const remove = async id => {
    ...
    try { await api.delete(`/notices/${id}`); load(); } catch (err) { alert(err.message); }
  };
    ...
    <DashboardLayout>
      ...
      {notices.length === 0 ? (
        ...
        <div className="space-y-4">
          {notices.map((n, i) => {
            ...
            return (
              <motion.div key={n.id} initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*.05 }}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4 flex-1">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5" style={{ background:'rgba(34,197,94,0.12)' }}>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <span className="px-2 py-0.5 rounded-full text-xs font-semibold flex items-center gap-1"
                          ...
                          <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background:p.dot }}/>{n.priority}
                        </span>
                      </div>
                  ...
                  {role==='admin' && (
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button onClick={() => openEdit(n)} className="p-2 rounded-lg transition-all" style={{ color:'var(--text3)' }}
                        onMouseEnter={e => { e.currentTarget.style.background='rgba(34,197,94,0.10)'; e.currentTarget.style.color='var(--p)'; }}
                        onMouseLeave={e => { e.currentTarget.style.background='transparent'; e.currentTarget.style.color='var(--text3)'; }}>
                        <Edit2 size={14}/>
                      </button>
                      <button onClick={() => remove(n.id)} className="p-2 rounded-lg transition-all" style={{ color:'var(--text3)' }}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Edit Notice' : 'Post Notice'}>
        <form onSubmit={submit} className="space-y-4">
          <div><label className="block mb-1.5" style={{ color:'var(--text3)', fontSize:'0.72rem', fontWeight:500 }}>Title *</label><input className="input-dark" placeholder="Notice title" value={form.title} onChange={set('title')} required/></div>
          <div><label className="block mb-1.5" style={{ color:'var(--text3)', fontSize:'0.72rem', fontWeight:500 }}>Description *</label><textarea className="input-dark resize-none" rows={4} placeholder="Notice details..." value={form.description} onChange={set('description')} required/></div>
          <div>
