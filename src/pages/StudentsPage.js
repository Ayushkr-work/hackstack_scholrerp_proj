...
import { getStudents, addStudent, updateStudent, deleteStudent } from '../utils/mockData';
...
const T = {
  ...
  row:   { borderBottom:'1px solid var(--border2)', transition:'background .15s' },
...
export default function StudentsPage() {
  ...
  const [students, setStudents] = useState(() => getStudents(cid));
  ...
  const refresh  = () => setStudents(getStudents(cid));
  const openAdd  = () => { setEditing(null); setForm(empty); setErr(''); setModal(true); };
  const openEdit = s  => { setEditing(s); setForm({ ...s, password:'' }); setErr(''); setModal(true); };

  const submit = e => {
    e.preventDefault(); setErr('');
    if (editing) {
      updateStudent(editing.id, { ...form, college_id: cid });
      refresh(); setModal(false);
    } else {
      const res = addStudent({ ...form, college_id: cid, semester: Number(form.semester) });
  ...
  return (
    <DashboardLayout>
      ...
      <div className="relative mb-6">
        ...
        <input className="input-dark input-icon-left" placeholder="Search by name, email, roll no..." value={search} onChange={e => setSearch(e.target.value)}/>
      </div>
      ...
      <div style={T.table}>
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.map((s, i) => (
                <motion.tr key={s.id} style={T.row}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Edit Student' : 'Add Student'}>
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={k}>
                ...
                <input className="input-dark" type={type} placeholder={ph} value={form[k]} onChange={set(k)} required={label.includes('*')}/>
              </div>
            ))}
            {!editing && (
              <div>
                <label className="block mb-1.5" style={T.label}>Password *</label>
                <input className="input-dark" type="password" placeholder="Set password" value={form.password} onChange={set('password')} required/>
              </div>
            )}
            <div>
          ...
          <button type="submit" className="btn-primary w-full mt-2">{editing ? 'Update Student' : 'Add Student'}</button>
        </form>
      </Modal>
    </DashboardLayout>
  );
