...
export default function PlacementsPage() {
  ...
  const submit = e => {
    ...
    if (editing) savePlacements(all.map(x => x.id===editing.id ? {...x,...form} : x));
    else savePlacements([{ id:getNextId(), ...form, college_id:cid, created_at:new Date().toISOString() }, ...all]);
            ...
            <motion.div key={p.id}
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center"
                  style={{ background:'linear-gradient(135deg,rgba(34,197,94,0.15),rgba(212,175,55,0.15))' }}>
                ...
                {role==='admin' && (
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => remove(p.id)} className="p-1.5 rounded-lg transition-all"
                      style={{ color:'var(--text3)' }}
              ...
              <div className="space-y-1.5 mb-4">
                {p.package && (
                  <div className="flex items-center gap-2" style={{ color:'var(--text3)', fontSize:'0.78rem' }}>
                    <DollarSign size={12} style={{ color:'var(--p)' }}/>{p.package}
                  </div>
                )}
                {p.deadline && (
                  <div className="flex items-center gap-2" style={{ color:'var(--text3)', fontSize:'0.78rem' }}>
                    <Calendar size={12} style={{ color:'var(--s)' }}/>Deadline: {new Date(p.deadline).toLocaleDateString()}
                  </div>
                )}
                {p.eligibility && (
                  <div className="flex items-start gap-2" style={{ color:'var(--text3)', fontSize:'0.78rem' }}>
                    <Briefcase size={12} style={{ color:'var(--text4)', marginTop:'2px', flexShrink:0 }}/>
              ...
              {role==='student' && (
                <button
                  onClick={() => setApplied(prev => new Set([...prev, p.id]))}
                  disabled={applied.has(p.id)}
                  className={applied.has(p.id) ? '' : 'btn-primary'}
                  style={applied.has(p.id) ? { width:'100%', padding:'0.6rem', borderRadius:'10px', fontSize:'0.85rem', fontWeight:600, background:'rgba(34,197,94,0.10)', color:'#15803D', border:'1px solid rgba(34,197,94,0.25)', cursor:'none' } : { width:'100%' }}>
