...
import { Plus, CreditCard, CheckCircle, Clock, Loader2 } from 'lucide-react';
...
export default function FeesPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api.get('/fees').then(d => setFees(Array.isArray(d) ? d : [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  const createFee = async e => {
    e.preventDefault();
    try { await api.post('/fees', { ...form, amount: Number(form.amount) }); load(); setModal(false); setForm({ student_id:'', amount:'', description:'', due_date:'' }); }
    catch (err) { alert(err.message); }
  };

  const simulatePay = async id => {
    try {
      await api.post('/fees/simulate-pay', { fee_id: id });
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6">
        ...
        {role==='admin' && <button onClick={() => setModal(true)} className="btn-primary flex items-center gap-2 text-sm"><Plus size={16}/> Add Fee</button>}
      </div>

      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="rounded-2xl p-5" style={{ background:'var(--bg3)', border:'1px solid rgba(34,197,94,0.20)', boxShadow:'var(--shadow)' }}>
          <div className="flex items-center gap-3 mb-2"><CheckCircle size={18} style={{ color:'var(--p)' }}/><span style={{ color:'var(--text2)', fontSize:'0.85rem' }}>Total Paid</span></div>
        ...
        <div className="rounded-2xl p-5" style={{ background:'var(--bg3)', border:'1px solid rgba(212,175,55,0.20)', boxShadow:'var(--shadow)' }}>
          <div className="flex items-center gap-3 mb-2"><Clock size={18} style={{ color:'var(--s)' }}/><span style={{ color:'var(--text2)', fontSize:'0.85rem' }}>Pending</span></div>
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom:'1px solid var(--border2)' }}>
                {[role==='admin'?'Student':null,'Description','Amount','Due Date','Status','Action'].filter(Boolean).map(h=>(
                  <th key={h} className="text-left px-6 py-4" style={th}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="text-center py-14"><Loader2 size={22} className="animate-spin mx-auto" style={{ color:'var(--p)' }}/></td></tr>
              ) : fees.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-14" style={{ color:'var(--text3)' }}><CreditCard size={38} style={{ margin:'0 auto 10px', opacity:.3 }}/><p style={{ fontSize:'0.85rem' }}>No fee records found</p></td></tr>
              ) : fees.map((f, i) => (
                <motion.tr key={f.id} style={{ borderBottom:'1px solid var(--border2)', transition:'background .15s' }}
                  initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:i*.03 }}
                  onMouseEnter={e => e.currentTarget.style.background='rgba(34,197,94,0.04)'}
                  onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                  {role==='admin' && <td className="px-6 py-4"><p style={{ color:'var(--text1)', fontWeight:600, fontSize:'0.85rem' }}>{f.student_name}</p><p style={{ color:'var(--text3)', fontSize:'0.75rem' }}>{f.roll_no}</p></td>}
                  <td className="px-6 py-4" style={{ color:'var(--text2)', fontSize:'0.85rem' }}>{f.description||'Tuition Fee'}</td>
                  <td className="px-6 py-4" style={{ color:'var(--text1)', fontWeight:700, fontSize:'0.9rem' }}>₹{Number(f.amount).toLocaleString()}</td>
                  <td className="px-6 py-4" style={{ color:'var(--text3)', fontSize:'0.85rem' }}>{f.due_date?new Date(f.due_date).toLocaleDateString():'—'}</td>
                  <td className="px-6 py-4"><span className={f.status==='paid'?'badge-paid':'badge-pending'}>{f.status}</span></td>
                  <td className="px-6 py-4">
                    {role==='student' && f.status==='pending' ? (
                      ...
                      <span className="flex items-center gap-1 text-xs font-semibold" style={{ color:'var(--p)' }}><CheckCircle size={12}/> Paid</span>
                    ) : null}
                  </td>
                </motion.tr>
              ))}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Add Fee Record">
        <form onSubmit={createFee} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block mb-1.5" style={lbl}>Amount (₹) *</label><input className="input-dark" type="number" placeholder="50000" value={form.amount} onChange={set('amount')} required/></div>
            <div><label className="block mb-1.5" style={lbl}>Due Date</label><input className="input-dark" type="date" value={form.due_date} onChange={set('due_date')}/></div>
          </div>
          <div><label className="block mb-1.5" style={lbl}>Description</label><input className="input-dark" placeholder="e.g. Semester 1 Tuition Fee" value={form.description} onChange={set('description')}/></div>
      ...
      <Modal open={!!payModal} onClose={() => setPayModal(null)} title="Confirm Payment" size="sm">
        {payModal && (
          <div className="text-center space-y-4">
            ...
            <div className="rounded-xl p-4 text-left space-y-2" style={{ background:'var(--bg4)', border:'1px solid var(--border2)' }}>
              <p style={{ color:'var(--text3)', fontSize:'0.75rem', fontWeight:500 }}>Select Payment Method</p>
              <div className="flex gap-2">
                {['UPI','Card','Net Banking'].map(m=>(
                  <div key={m} className="flex-1 py-2 rounded-lg text-center text-xs font-medium" style={{ background:'var(--bg3)', border:'1px solid var(--border2)', color:'var(--text2)' }}>{m}</div>
                ))}
              </div>
            </div>
